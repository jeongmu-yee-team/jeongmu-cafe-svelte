<script lang="ts">
  ...
  const itemsPerPage = 8; // 페이지 당 상품 카드 수
    ...
    {#if $filteredData}
      <div class=" card-container grid h-full grid-cols-4 grid-rows-2 gap-x-6 gap-y-10 xl:gap-x-8">
        {#each $filteredData.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage) as item}
          <div
            class="active:transtion group rounded-lg border-2 text-center duration-75 ease-out active:scale-90
            active:shadow-2xl"
          >
            <div
              class="aspect-h-1 aspect-w-1 xl:aspect-h-8 xl:aspect-w-7 w-full overflow-hidden rounded-b-lg bg-gray-200"
            >
              <img
            ...
            <p class="mt-2 pb-4 font-semibold text-orange-600">{item.product_price}원</p>
          </div>
        {/each}
      </div>
