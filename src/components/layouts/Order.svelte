<script lang="ts">
  ...
  export let removeFromCart: (productName: string) => void;
...
<div class="max-h-96 overflow-y-auto">
  {#if cart.length > 0}
    <ul>
      {#each cart as { product_name, product_price, product_thumbnail_url, quantity }, index (index)}
        <li class="flex items-center justify-between p-2">
          ...
          <button
            on:click={() => removeFromCart(product_name)}
            class="rounded bg-gray-400 px-4 py-2 font-bold text-white hover:bg-red-700"
          >
            x
          </button>
        </li>
